...
class Tetris():
    ...
    def deleteFullLines(self): # To be implemented!!
        checkLines = self.currBlk.get_dy()

        if self.top + self.currBlk.get.dy() >= self.iScreenDy :
            checkLines = self.iScreenDy - self.top

        rmv = 0
    
        arrayScreen = self.createArrayScreen()
        newScreen = Matrix(arrayScreen)
        blank = new_fullScreen.clip(0, 0, 1, new_fullScreen.get_dx())
    
        for i in range(checkLines-1, -1, -1):
            checkLayer = self.top + i + rmv
            line = self.oScreen.clip(checkLayer, 0, checkLayer + 1, self.oScreen.get_dx())
            if line.sum() == self.oScreen.get_dx():
                tmp = self.oScreeen.clip(0, 0, checkLines, self.oScreen.get_dx())
                self.oScreen.paste(tmp, 1, 0)
                self.oScreen.paste(blank, 0, 0)
                rmv += 1
        return self.oScreen
    ...
    def accept(self, key): # To be implemented!!
        ...
        if key == 'a':
            ...
            while (1):
                self.top += 1
                self.tempBlk self.iScreen.clip(self.top, self.letf, self.top + self.currBlk.get_dy(), self.left + self.currBlk.get_dx())
